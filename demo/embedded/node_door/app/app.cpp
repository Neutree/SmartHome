	...
	if(com2.ReceiveBufferSize()>5)
	{
		...
		if(size>0)
		{
			...
			if(dataType == Protocol::Switch::dataType)
			{
				...
				{
					if(*((Protocol::Switch*)data)->comment == 1)//门
					{
...
//					}
			...
			else if(dataType == Protocol::Sensor::dataType)
			{
				...
				{
//					char sensorNumber[]={0,0,0,1};
//					if(((Protocol::Sensor*)data)->name == 1)//火焰传感器
//					{
//						SendSensorInfoToServer(sensorNumber,mFireSensor);
//					}
//					else if(((Protocol::Sensor*)data)->name == 2){//烟雾传感器
//						sensorNumber[3] = 2;
//						SendSensorInfoToServer(sensorNumber,mSmokeSensor);
//					}
				}//询问开关状态
			}
...
bool App::SendSensorInfoToServer(char* sensorNumber,char value)
{
	mToServer.operationType = Protocol::OperationType_Ack;
	mToServer.dataType = Protocol::Sensor::dataType;
	mToServer.dataLength = 8;
	memcpy(mToServer.data,sensorNumber,4);
	mToServer.data[4] = 1;
	mToServer.data[5] = value;
	mToServer.data[6]=0;
	mToServer.data[7]=0;
	if(Write(mToServer))
	{
